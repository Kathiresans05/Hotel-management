...
const CheckIn = () => {
  ...
  const steps = [
    { id: 1, name: t('SELECT_ROOM'), icon: Bed },
    { id: 2, name: t('CUSTOMER_DETAILS'), icon: User },
    { id: 3, name: t('ADD_SERVICES'), icon: Plus },
    { id: 4, name: t('BILLING_PAYMENT'), icon: Receipt },
  ];

  return (
    <div className="animate-in fade-in duration-500 max-w-5xl mx-auto">
      <PageHeader 
        title={t('QUICK_CHECK_IN')} 
        subtitle={t('CHECKIN_DESC')}
      />
          ...
          <div className="bg-surface p-8 rounded-card border border-border">
            {step === 1 && (
              <div className="animate-in slide-in-from-right-4 duration-300">
                <h3 className="text-xl font-bold text-text-primary mb-6">{t('SELECT_AVAILABLE_ROOM')}</h3>
                  ...
                  {['101', '103', '107', '201', '205', '302'].map((num) => (
                    <div key={num} className="p-4 border-2 border-border rounded-xl hover:border-accent cursor-pointer transition-all group active:scale-95">
                      <div className="flex justify-between items-center">
                        <span className="text-lg font-bold text-text-primary">{t('ROOM')} {num}</span>
                        <span className="text-xs font-bold text-success bg-success/10 px-2 py-0.5 rounded">{t('AVAILABLE')}</span>
                      </div>
                      <p className="text-xs text-text-secondary mt-1">{t('SINGLE_DELUXE')} • {t('FLOOR')} {num.charAt(0)}</p>
                      <p className="text-sm font-bold text-accent mt-3">$120 <span className="text-[10px] text-text-secondary">/ {t('NIGHT')}</span></p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {step === 2 && (
              <div className="animate-in slide-in-from-right-4 duration-300 space-y-4">
                <h3 className="text-xl font-bold text-text-primary mb-6">{t('GUEST_INFORMATION')}</h3>
                <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-text-secondary uppercase">{t('ID_TYPE')}</label>
                    <select className="w-full px-4 py-2.5 bg-background border border-border rounded-input text-text-primary focus:border-accent/40 outline-none">
                      ...
                      <option>{t('DRIVERS_LICENSE')}</option>
                      <option>{t('NATIONAL_ID')}</option>
                    </select>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-text-secondary uppercase">{t('ID_NUMBER')}</label>
            ...
            {step === 3 && (
              <div className="animate-in slide-in-from-right-4 duration-300">
                <h3 className="text-xl font-bold text-text-primary mb-6">{t('ADDITIONAL_SERVICES')}</h3>
                <div className="space-y-3">
                  {[
                    { name: t('BREAKFAST'), price: 15, desc: t('BREAKFAST_DESC') },
                    { name: t('LAUNDRY'), price: 20, desc: t('LAUNDRY_DESC') },
                    { name: t('AIRPORT_SHUTTLE'), price: 40, desc: t('AIRPORT_SHUTTLE_DESC') },
                    { name: t('EXTRA_BED'), price: 30, desc: t('EXTRA_BED_DESC') }
                  ].map((service) => (
            ...
            {step === 4 && (
              <div className="animate-in slide-in-from-right-4 duration-300">
                <h3 className="text-xl font-bold text-text-primary mb-6">{t('PAYMENT_METHOD')}</h3>
                <div className="grid grid-cols-3 gap-4 mb-8">
                  {[
                    { id: 'cash', name: t('CASH'), icon: Receipt },
                    { id: 'card', name: t('CARD'), icon: CreditCard },
                    { id: 'upi', name: t('UPI'), icon: DollarSign }
                  ].map((method) => (
                ...
                <div className="bg-success-soft p-6 rounded-2xl border border-success/20">
                  <div className="flex justify-between items-center">
                    <div>
                      <p className="text-xs font-bold text-success uppercase">{t('TOTAL_AMOUNT_DUE')}</p>
        ...
        <div className="lg:col-span-1">
          <div className="bg-surface rounded-card border border-border overflow-hidden sticky top-24">
            <div className="bg-primary p-6 text-white">
              <h4 className="font-bold text-sm uppercase tracking-widest opacity-60">{t('BOOKING_SUMMARY')}</h4>
              <p className="text-xs mt-1">{t('REF')}: #NEW-CHECKIN</p>
            </div>
            <div className="p-6 space-y-6">
              <div>
                <div className="flex justify-between text-xs font-bold text-text-secondary uppercase mb-3">
                  <span>{t('ROOM_DETAILS')}</span>
                  <span className="text-accent cursor-pointer hover:underline">{t('EDIT')}</span>
                </div>
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm font-bold text-text-primary">{t('ROOM')} 101</p>
                    <p className="text-[10px] text-text-secondary">{t('DELUXE_SINGLE')} • 3 {t('NIGHTS')}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-border pt-6">
                <div className="flex justify-between text-xs font-bold text-text-secondary uppercase mb-3">
                  <span>{t('BILL_BREAKDOWN')}</span>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-text-secondary">{t('ROOM_CHARGE')}</span>
                    <span className="font-medium text-text-primary">$360.00</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-text-secondary">{t('SERVICES')}</span>
                    <span className="font-medium text-text-primary">$55.00</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-text-secondary">{t('GST')} (10%)</span>
